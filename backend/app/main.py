...
async def lifespan(app: FastAPI):
    ...
    # On startup: refresh the sector performance chart too, rather than waiting up to a day
    # for the next scheduled PgQueuer run (non-blocking task).
    try:
        await get_pgq_queries().enqueue("refresh_sector_performance", payload=b"startup")
    except Exception:
        pass  # Don't block startup if the job queue is unavailable

    # On startup: refresh the equity risk premium chart too, rather than waiting up to a day
    # for the next scheduled PgQueuer run (non-blocking task).
    try:
        await get_pgq_queries().enqueue("refresh_equity_premium", payload=b"startup")
    except Exception:
        pass  # Don't block startup if the job queue is unavailable
