...
async def _get_latest_price_rows(
    db: AsyncSession, tickers: list[str], as_of: Optional[date] = None
) -> dict[str, AssetPrice]:
    """Returns {ticker: AssetPrice} for the latest known price row of each ticker, or the
    latest at or before `as_of` when given. Like _get_latest_prices, but keeps the full row
    (date, source included) for callers that need that metadata too, instead of re-querying it
    separately."""
    if not tickers:
        return {}
    where_clauses = [AssetPrice.ticker.in_(tickers)]
    if as_of is not None:
